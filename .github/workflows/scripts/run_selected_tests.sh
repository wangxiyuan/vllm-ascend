...
setup_vllm_upstream() {
  ...
  if ! grep -q "vllm_ascend.patch.platform" "${vllm_dir}/tests/conftest.py"; then
    # Insert the platform patches right before the first import so they load
    # ahead of any vLLM module. Line-number-based insertion would silently
    # break when the upstream conftest header changes; awk keeps this
    # portable across BSD/GNU userlands.
    local patch_tmp
    patch_tmp="$(mktemp)"
    awk '
      /^[[:space:]]*(import|from)[[:space:]]/ && !inserted {
        print "import vllm_ascend.patch.platform"
        print "import vllm_ascend.patch.worker"
        inserted = 1
      }
      { print }
    ' "${vllm_dir}/tests/conftest.py" > "${patch_tmp}"
    cat "${patch_tmp}" > "${vllm_dir}/tests/conftest.py"
    rm -f "${patch_tmp}"
  fi
