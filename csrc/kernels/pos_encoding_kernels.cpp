...
template <typename scalar_t, bool isNeox> class RotaryEmbedding {
  // NOTE(ganyi): we use 32K as load stride for pipe, need to find another way
  // to retrieve this size from runtime for more Soc support
...
public:
  ...
  // Allocate buffers for input and output queue and the temp buffer used during
  // kernel compute process, this init process happens only in the kernel
  // compute on a single vector core.
  __aicore__ inline void
  init(__gm__ int64_t *positions, __gm__ void *queryDst, __gm__ void *keyDst,
       __gm__ scalar_t *query, __gm__ scalar_t *key,
       __gm__ scalar_t *cosSinCache, const int rotDim,
       const int64_t dstQueryStride, const int64_t dstKeyStride,
       const int64_t queryStride, const int64_t keyStride, const int numHeads,
       const int numKvHeads, const int headSize, AscendC::TPipe *pipe) {
    pipe_ = pipe;
    rotDim_ = rotDim;
    // query stride and key stride is used to handle the strided tensor which is
    // not contiguous on num_tokens dim
    ...
    pipe_->InitBuffer(inQueSinCos_, 1 /* buffer_num */,
                      rotDim_ * sizeof(scalar_t) /* buffer_size */);
    ...
    upcastInputBufferOffset_ =
        calcTmpBufferOffset_ + sizeof(acc_t) * embedDim_ * 2;
    // 1 upcast temp buffer for bf16 (2 * embed_dim)
    upcastTempBufferOffset_ =
        upcastInputBufferOffset_ + sizeof(acc_t) * headSize_;
    // 2 sin cos upcast buffer for bf16
    cosSinUpcastBufferOffset_ =
        upcastTempBufferOffset_ + sizeof(acc_t) * 2 * embedDim_;
    ...
    // need to consider upcast the bf16 to fp32, so we might need 4 buffer just
    // in case 2 temporary buffer, 2 input buffer, 1 cos buffer, 1 sin buffer, 2
    // scale buffer (headSize), 2 zp buffer(headSize int8), 1 dst_temp
    // buffer(headSize, int32)
  ...
  __aicore__ inline void update_mem_offset(
      __gm__ int64_t *positions, __gm__ void *queryDst, __gm__ void *keyDst,
      __gm__ scalar_t *query, __gm__ scalar_t *key,
      __gm__ scalar_t *cosSinCache, const int rotDim,
      const int64_t dstQueryStride, const int64_t dstKeyStride,
      const int64_t queryStride, const int64_t keyStride, const int numHeads,
    ...
    queryDst_.SetGlobalBuffer(reinterpret_cast<__gm__ dst_t *>(queryDst) +
                                  dstQueryStride * idx,
                              headSize * numHeads_);
    keyDst_.SetGlobalBuffer(reinterpret_cast<__gm__ dst_t *>(keyDst) +
                                dstKeyStride * idx,
                            headSize * numKvHeads_);
  }

  // compute per head for neox on bf16
  template <typename acc_t_,
            typename std::enable_if<!std::is_same_v<acc_t_, scalar_t>,
                                    void>::type * = nullptr>
  __aicore__ inline void
  neox_compute(local_scalar_t src, local_dst_t dst,
               AscendC::LocalTensor<acc_t_> sin,
               AscendC::LocalTensor<acc_t_> cos,
               AscendC::LocalTensor<acc_t_> upcastInputBuffer,
               AscendC::LocalTensor<acc_t_> calcTmpBuffer) {
    // slice dst
  ...
  __aicore__ inline void compute_qk(AscendC::GlobalTensor<scalar_t> srcG,
                                    AscendC::GlobalTensor<dst_t> dstG,
                                    local_acc_t localCos, local_acc_t localSin,
                                    local_acc_t upcastInputBuffer,
                                    local_acc_t calcTmpBuffer, int loopCnt,
                                    int tailHeads, int loadStride,
                                    int headNumPerLoad) {
    for (int loopNum = 0; loopNum < loopCnt; ++loopNum) {
      ...
      if constexpr (!std::is_same_v<scalar_t, acc_t>) {
        ...
        AscendC::LocalTensor<acc_t> upBuffer =
            copyBuf_.GetWithOffset<acc_t>(elem_num, 0);
      ...
      for (int i = 0; i < headNumPerLoad; ++i) {
        neox_compute(srcDeque[i * headSize_], dst[i * headSize_], localSin,
                     localCos, upcastInputBuffer, calcTmpBuffer);
      }
      outQue_.EnQue(dst);
    ...
    // process tail
    {
      ...
      outQue_.EnQue(dst);
      local_dst_t dstDeque = outQue_.DeQue<dst_t>();
      AscendC::DataCopy(dstG[loopCnt * loadStride], dstDeque,
                        tailHeads * headSize_);
      outQue_.FreeTensor(dstDeque);
      inQue_.FreeTensor(srcDeque);
    }
  ...
  __aicore__ inline void compute_function() {
    ...
    calcTmpBuffer =
        calcBuf_.GetWithOffset<acc_t>(embedDim_ * 2, calcTmpBufferOffset_);
    upcastInputBuffer =
        calcBuf_.GetWithOffset<acc_t>(headSize_, upcastInputBufferOffset_);
    upcastTempBuffer =
        calcBuf_.GetWithOffset<acc_t>(embedDim_ * 2, upcastTempBufferOffset_);
    cosSinUpcastBuffer =
        calcBuf_.GetWithOffset<acc_t>(embedDim_ * 2, cosSinUpcastBufferOffset_);

    local_acc_t cosAccBuffer;
    local_acc_t sinAccBuffer;

    if constexpr (!std::is_same_v<scalar_t, acc_t>) {
      Cast(cosSinUpcastBuffer, localSinCosDeque, AscendC::RoundMode::CAST_NONE,
           2 * embedDim_);
    ...
    compute_qk(key_, keyDst_, cosAccBuffer, sinAccBuffer, upcastInputBuffer,
               calcTmpBuffer, loopCntKv, tailHeadsKv, loadStride,
               headNumPerLoad);
...
// Note: Need to use macro to instaniate all the target functions here, for the
// current build system dose not support template call in cpp We use C style
// symbol here for kernel compilation, cpp style kernel entry may lead to
// compilation failure
#define ROPE_CUSTOM_KERNEL_TYPE_DECLARE(TYPE, NEOX)                            \
  extern "C" __global__ __aicore__ void rope_custom_##NEOX##_##TYPE(           \
      __gm__ int64_t *positions, __gm__ void *queryDst, __gm__ void *keyDst,   \
      __gm__ TYPE *query, __gm__ TYPE *key, __gm__ TYPE *cosSinCache,          \
      const int rotDim, const int64_t queryStride, const int64_t keyStride,    \
      const int64_t dstQueryStride, const int64_t dstKeyStride,                \
      const int numHeads, const int numKvHeads, const int headSize,            \
      const int64_t numTokens, const int loopNum, const int coreNum) {         \
    AscendC::TPipe pipe;                                                       \
    RotaryEmbedding<TYPE, NEOX> op{};                                          \
    op.init(positions, queryDst, keyDst, query, key, cosSinCache, rotDim,      \
            dstQueryStride, dstKeyStride, queryStride, keyStride, numHeads,    \
            numKvHeads, headSize, &pipe);                                      \
    for (int64_t i = AscendC::GetBlockIdx(); i < numTokens; i += coreNum) {    \
      op.update_mem_offset(positions, queryDst, keyDst, query, key,            \
                           cosSinCache, rotDim, dstQueryStride, dstKeyStride,  \
                           queryStride, keyStride, numHeads, numKvHeads,       \
                           headSize, i);                                       \
      op.compute_function();                                                   \
    }                                                                          \
  }
...
#define ROTARY_EMBEDDING_KERNEL_CALL(TYPE)                                     \
  if (isNeox)                                                                  \
    rope_custom_true_##TYPE<<<blockDim, nullptr, stream>>>(                    \
        positions, queryDst, keyDst, reinterpret_cast<TYPE *>(query),          \
        reinterpret_cast<TYPE *>(key), reinterpret_cast<TYPE *>(cosSinCache),  \
        rotDim, queryStride, keyStride, dstQueryStride, dstKeyStride,          \
...
extern void rotary_embedding_impl(
    AscendType type, bool isNeox, void *stream, int64_t *positions,
    void *queryDst, void *keyDst, void *query, void *key, void *cosSinCache,
    const int rotDim, const int64_t queryStride, const int64_t keyStride,
    const int64_t dstQueryStride, const int64_t dstKeyStride,
    const int numHeads, const int numKvHeads, const int headSize,
    const int64_t numTokens, const uint32_t loopCnt, uint32_t aivNum) {
