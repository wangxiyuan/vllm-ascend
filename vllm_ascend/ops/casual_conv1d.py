...
def causal_conv1d_ref2(
    ...
    if initial_states is None:
        ...
        x = torch.cat([initial_states, x.unsqueeze(0)], dim=-1)
        out = F.conv1d(x, weight.unsqueeze(1), bias, padding=0, groups=dim)
    out = out[..., :seqlen]
    if return_final_states:
        final_states = F.pad(x, (width - 1 - x.shape[-1], 0)).to(
...
def causal_conv1d_fn(
    ...
    out_ref_b.append(
        causal_conv1d_ref(
            ...
            initial_states=conv_states[cache_indices[0]] if has_initial_state[0] else None))
...
def causal_conv1d_update(
    ...
    #     pad_slot_id,
    # )
    x = causal_conv1d_update_ref(
        x, conv_state[conv_state_indices], weight, bias, activation=activation
    )
    if unsqueeze:
        x = x.squeeze(-1)
    return x
