...
class NPUPlatform(Platform):
    ...
    def check_and_update_config(cls, vllm_config: VllmConfig) -> None:
        ...
        # 4. compilation config update
        ascend_compilation_config = ascend_config.ascend_compilation_config
        if ascend_compilation_config:
            if ascend_compilation_config.fuse_allreduce_rms:
                new_compile_ranges_split_points = compilation_config.compile_ranges_split_points
                ...
                compilation_config.compile_ranges_split_points = new_compile_ranges_split_points
                logger.debug(
        ...
        # 6. Update compilation mode in some cases
        if enforce_eager:
        ...
        # 7. Update cudagraph_mode in some cases
        ...
        if compilation_config.cudagraph_mode == CUDAGraphMode.FULL_AND_PIECEWISE:
            logger.warning("FULL_AND_PIECEWISE mode is not supported on NPU, fallback to PIECEWISE ")
        ...
        # 8. set cudaprah sizes before extending `compilation_config.splitting_ops`
        vllm_config._set_cudagraph_sizes()

        # 9. Set and update for different cudagraph_mode
        if compilation_config.cudagraph_mode == CUDAGraphMode.NONE:
            compilation_config.mode = CompilationMode.NONE
            ascend_config.npugraph_ex_config.enable = False
        elif compilation_config.cudagraph_mode == CUDAGraphMode.PIECEWISE:
            logger.info("PIECEWISE compilation enabled on NPU.")
            ...
            logger.info(f"{compilation_config.cudagraph_mode} compilation enabled on NPU.")
            compilation_config.use_inductor = False
            compilation_config.splitting_ops = []
            ...
            **********************************************************************************\033[0m
            ...
            logger.info(
                f"{compilation_config.cudagraph_mode} cudagraph_mode is not support on NPU. Falling back to NONE"
            )
            compilation_config.cudagraph_mode = CUDAGraphMode.NONE
            compilation_config.mode = CompilationMode.NONE
            ascend_config.npugraph_ex_config.enable = False

        # 10. check ASCEND_LAUNCH_BLOCKING
        ...
        if (
            ...
            raise ValueError(
                "ACL graph is incompatible with ASCEND_LAUNCH_BLOCKING=1. Please unset ASCEND_LAUNCH_BLOCKING or set "
                "it to 0. If you need ASCEND_LAUNCH_BLOCKING for debugging, consider other methods — for example, "
                "check the plog files (default: $HOME/ascend/log/debug) for more information about runtime errors."
            )

        # 11. update graph capture sizes for sequence parallelism
        # TODO delete graph size update here when compilation_config.pass_config.enable_sp
        # is supported by vllm-ascend.
        if parallel_config.tensor_parallel_size > 1 and not model_config.enforce_eager and enable_sp(vllm_config):
        ...
        # 12. refresh block_size for some special cases
        refresh_block_size(vllm_config)

        # 13. Activate custom ops, except on 310P
        if get_ascend_device_type() != AscendDeviceType._310P:
            compilation_config.custom_ops = ["all"]

        # 14. Set PYTORCH_NPU_ALLOC_CONF=expandable_segments:True to optimize NPU memory management.
        ...
        # 15. some special cases check
        # 1). check and update cp_kv_cache_interleave_size
        # 2). check VLLM_ASCEND_ENABLE_FLASHCOMM1
        # 3). check speculative_config eager mode
        ...
        if is_vl_model(vllm_config) and ascend_envs.VLLM_ASCEND_ENABLE_FLASHCOMM1:
            raise ValueError(
                "Currently, VL models doesn't support FLASHCOMM in vllm-ascend. We will fix this in the future. "
        ...
        if (
            ...
            and vllm_config.speculative_config.enforce_eager
        ):
...
def _fix_incompatible_config(vllm_config: VllmConfig) -> None:
    ...
    if vllm_config.cache_config:
        ...
        if getattr(vllm_config.cache_config, "cpu_kvcache_space_bytes", False):
            logger.warning("Parameter 'cpu_kvcache_space_bytes' is tied to cpu backend. Resetting to None for Ascend.")
    ...
    if vllm_config.kv_transfer_config:
        ...
        if getattr(vllm_config.kv_transfer_config, "enable_permute_local_kv", False):
            logger.warning(
                "Parameter 'enable_permute_local_kv' is tied to NIXL backend. Resetting to False for Ascend stability."
            )
    ...
    if vllm_config.attention_config:
        ...
        if getattr(att_config, "flash_attn_max_num_splits_for_cuda_graph", 32) != 32:
            logger.warning(
                "Parameter 'flash_attn_max_num_splits_for_cuda_graph' is ignored on Ascend. Resetting to default (32)."
            )
